				...
				<div class="panel-body">@if (session('status'))
					...
					<form class="form-horizontal" method="POST" action="{{ route('password.email') }}">{{ csrf_field() }}
						...
						<div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
							...
							<div class="col-md-6">
								<input id="email" type="email" class="form-control input1" name="email" value="{{ $email or old('email') }}" required autofocus>@if ($errors->has('email')) <span class="help-block">
						...
						<div class="form-group{{ $errors->has('password_confirmation') ? ' has-error' : '' }}">
							...
							<div class="col-md-6">
								<input id="password-confirm" type="password" class="form-control input1" name="password_confirmation" required>@if ($errors->has('password_confirmation')) <span class="help-block">
						...
						<div class="form-group">
							<div class="col-md-6 col-md-offset-4">
								<button type="submit" class="btn btn-primary button1">Reset Password</button>
							</div>
						</div>
					</form>
				</div>
